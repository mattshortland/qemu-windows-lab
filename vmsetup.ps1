...
$subnet = "192.168.3.0/24" #Format Network/Mask e.g 192.168.1.0/24
$netbiosname = "TEST" #Max 16 Characters
...
$VirtInstallArgs = '--cpu EPYC' # Add any additional arguments 
...
if (!(test-path $PSScriptroot/ISO/pfsense.iso))
    {
        start-process -FilePath "gunzip" -ArgumentList "-k $psscriptroot/Toolkit/Downloads/pfSense-CE-2.4.5-RELEASE-p1-amd64.iso.gz" -Wait
...
"PRIMARYDCMAC" { 
   ...
   $action = New-ScheduledTaskAction -Execute 'Powershell.exe'-Argument '-NoProfile -WindowStyle Hidden -File c:\Setup\Scripts\PrimaryDC-Setup-2.ps1'
...
"SECONDARYDCMAC" { 
    ...
    $cred = ConvertTo-SecureString "ADMINISTRATORPASSWORDVALUE" -AsPlainText -Force
    $action = New-ScheduledTaskAction -Execute 'Powershell.exe'-Argument '-NoProfile -WindowStyle Hidden -File c:\Setup\Scripts\SecondaryDC-Setup-2.ps1'
    $trigger =  New-ScheduledTaskTrigger -AtLogon
    $User = "Administrator"
    Register-ScheduledTask -Action $action -Trigger $trigger -User $User -TaskName "SecondLogon" -Description "Second Logon Script"
...
"SCCMSERVERMAC" {
   ...
   $adapter | New-NetIPAddress -AddressFamily $IPType -IPAddress $IP -PrefixLength $MaskBits -DefaultGateway $Gateway
   $adapter | Set-DnsClientServerAddress -ServerAddresses $DNS
   $action = New-ScheduledTaskAction -Execute 'Powershell.exe'-Argument '-NoProfile -WindowStyle Hidden -File c:\Setup\Scripts\SCCMServer-Setup-2.ps1'
   $trigger =  New-ScheduledTaskTrigger -AtLogon
...
"MEMBERSERVERMAC" {
    ...
    if ($DHCPTrue -ne "DHCP")
        ...
        $action = New-ScheduledTaskAction -Execute 'Powershell.exe'-Argument '-NoProfile -WindowStyle Hidden -File c:\Setup\Scripts\MemberServer-Setup-2.ps1'
        $trigger =  New-ScheduledTaskTrigger -AtLogon
        Register-ScheduledTask -Action $action -Trigger $trigger -User Administrator -TaskName "SecondLogon" -Description "Second Logon Script"
        $domainpassword = ConvertTo-SecureString "ADMINISTRATORPASSWORDVALUE" -AsPlainText -Force
        $DomainCred = New-Object System.Management.Automation.PSCredential ("NETBIOSDOMAINNAME\Administrator", $domainpassword)
...
"WORKSTATIONMAC" {
    ...
    if ($DHCPTrue -ne "DHCP")
        ...
        $action = New-ScheduledTaskAction -Execute 'Powershell.exe'-Argument '-NoProfile -WindowStyle Hidden -File c:\Setup\Scripts\Workstation-Setup-2.ps1'
        $trigger =  New-ScheduledTaskTrigger -AtLogon
        Register-ScheduledTask -Action $action -Trigger $trigger -User Administrator -TaskName "SecondLogon" -Description "Second Logon Script"
        $domainpassword = ConvertTo-SecureString "ADMINISTRATORPASSWORDVALUE" -AsPlainText -Force
        $DomainCred = New-Object System.Management.Automation.PSCredential ("NETBIOSDOMAINNAME\Administrator", $domainpassword)
...
$action = New-ScheduledTaskAction -Execute 'Powershell.exe'-Argument '-NoProfile -WindowStyle Hidden -File c:\Setup\Scripts\Workstation-Setup-3.ps1'
$trigger =  New-ScheduledTaskTrigger -AtLogon
...
-replace 'VNETMASK', "$vnetmask" `
-replace 'ADMINISTRATORPASSWORDVALUE', $AdministratorPasswordValue
$PrimaryDCSecondLogonScript | out-file "$PSScriptroot/Toolkit/Scripts/PrimaryDC-Setup-2.ps1" -Force
}
...
-replace 'VNETMASK', "$vnetmask" `
-replace 'ADMINISTRATORPASSWORDVALUE', $AdministratorPasswordValue
$PrimaryDCThirdLogonScript | out-file "$PSScriptroot/Toolkit/Scripts/PrimaryDC-Setup-3.ps1" -Force
}
...
$PrimaryDCSecondLogonScript | out-file "$PSScriptroot/Toolkit/Scripts/SecondaryDC-Setup-2.ps1" -Force
...
$SCCMServerSecondLogonScript | out-file "$PSScriptroot/Toolkit/Scripts/SCCMServer-Setup-2.ps1" -Force
...
-replace 'VNETMASK', "$vnetmask" `
-replace 'ADMINISTRATORPASSWORDVALUE', $AdministratorPasswordValue
$MemberserverSecondLogonScript | out-file "$PSScriptroot/Toolkit/Scripts/MemberServer-Setup-2.ps1" -Force
...
$WorkstationSecondLogonScript | out-file "$PSScriptroot/Toolkit/Scripts/Workstation-Setup-2.ps1" -Force
